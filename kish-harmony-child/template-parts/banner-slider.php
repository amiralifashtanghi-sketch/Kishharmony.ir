<?php
/**
 * Module 3: Banner Slider
 */

$slider_interval = kishharmony_get_option('slider_interval', '4000');
?>

<div class="slider-wrapper max-w-7xl mx-auto my-8 px-4">
    <div class="slider-container relative rounded-[18px] overflow-hidden shadow-[0_12px_30px_rgba(0,0,0,0.25)]" id="heroSlider" data-interval="<?php echo esc_attr($slider_interval); ?>">
        <div class="slider flex transition-transform duration-700 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] will-change-transform" id="sliderTrack">
            <?php for ($s = 1; $s <= 3; $s++) {
                ...
                <div class="slide min-w-full h-[260px] sm:h-[320px] md:h-[420px] bg-cover bg-center relative flex items-center justify-center p-6 text-white" style="background-image: url('<?php echo esc_url($img); ?>');">
                    <!-- Dark overlay for WCAG contrast -->
                    <div class="absolute inset-0 bg-black/40 backdrop-brightness-[0.85]"></div>
                    <div class="slide-content relative z-10 text-center max-w-xl bg-white/10 backdrop-blur-[4px] p-6 rounded-2xl border border-white/15 shadow-lg">
                        ...
                        <a href="<?php echo esc_url($btn_link); ?>" class="inline-block bg-[#FF8A00] hover:bg-[#e07800] text-white text-xs sm:text-sm font-bold px-6 py-2.5 rounded-full shadow-md hover:shadow-lg transition-all">
        ...
        <div class="slider-dots absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20" id="sliderDots">
            <span class="dot w-3 h-3 rounded-full bg-white/50 cursor-pointer transition-all active bg-white w-6"></span>
            <span class="dot w-3 h-3 rounded-full bg-white/50 cursor-pointer transition-all"></span>
            <span class="dot w-3 h-3 rounded-full bg-white/50 cursor-pointer transition-all"></span>
        </div>
    </div>
</div>
